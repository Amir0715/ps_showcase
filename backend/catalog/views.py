...
class IndexView(ListView):
    ...
    def get_queryset(self):
        ...
        self.soon_games = list(q.filter(category__name__icontains='Скоро в продаже'))[:4]
        self.carousel_games = list(q.filter(incarousel=True))
        self.banner_games = list(q.filter(inbanner=True))[:2]
        self.game_category = list(Category.objects.filter(parent_category__name__icontains='Игры'))
        return q
...
class GameDetailView(View):
    ...
    def get_context_data(self, **kwargs):
        context = {}
        context["game_category"] = Category.objects.filter(parent_category__name__icontains='Игры')
...
class GameListView(ListView):
    ...
    def get_queryset(self):
        ...
        self.game_category = Category.objects.filter(parent_category__name__icontains='Игры')
        self.category = Category.objects.get(slug=self.kwargs.setdefault('category', 'games'))
