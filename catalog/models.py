...
class Gallery(models.Model):
    ...
    product = models.ForeignKey('Product', verbose_name='Продукт', related_name='images', on_delete=CASCADE)
...
class Product(models.Model):
    ...
    """
    category = models.ManyToManyField('Category', verbose_name='Категория') # игры, например
    ...
    incarousel = models.BooleanField(verbose_name='Показывать в карусели', default=False)
...
class CartProduct(models.Model):
    ...
    qty = models.PositiveIntegerField(default=1) # кол-во продукта одного наименования, например 10 яйиц
...
class Cart(models.Model):
    '''
        Модель корзины
    '''
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name='Владелец', on_delete=CASCADE) # владелец корзины
    products = models.ManyToManyField('CartProduct', blank=True, related_name='related_cart') # карточки продуктов
    total_products = models.PositiveIntegerField(default=0) # кол-во наименований продукта в корзине, например: яйца, хлеб, рис - 3 
...
class CategoryAttribute(models.Model):
    ...
    '''
    category = models.ForeignKey("Category", verbose_name="Атрибут категории", on_delete=models.CASCADE, related_name="attributes") # категория
    name = models.CharField(verbose_name="Название атрибута", max_length=255) # названия атрибута, то бишь ключ
...
class AttributeValue(models.Model):
    ...
    '''
    attribute = models.ForeignKey("CategoryAttribute", verbose_name="Атрибут", on_delete=models.CASCADE, related_name="values") # атрибут, ключ
    product = models.ForeignKey("Product", verbose_name="Значение атрибута для продукта", on_delete=models.CASCADE, related_name="values") # продукт
    value = models.CharField(verbose_name="Значение атрибута", max_length=255) # значение атрибута
